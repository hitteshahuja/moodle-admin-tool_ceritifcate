...
class issues extends \external_api {
    ...
    public static function regenerate_issue_file($issueid) {
        ...
        // Recalculate expiry date from the attached course module certificate, or reset to 0.
        $expires = 0;
        if ($issue->component === 'mod_coursecertificate' && !empty($issue->courseid)) {
            $cm = get_coursemodule_from_instance('coursecertificate', $issue->courseid);
            if ($cm) {
                $coursecertificate = $DB->get_record('coursecertificate', ['id' => $cm->instance]);
                if ($coursecertificate) {
                    $expires = \tool_certificate\certificate::calculate_expirydate(
                        (int) $coursecertificate->expirydatetype,
                        (int) $coursecertificate->expirydateoffset,
                        (int) $coursecertificate->expirydateoffset
                    );
                }
            }
        }
        $DB->set_field('tool_certificate_issues', 'expires', $expires, ['id' => $issue->id]);
